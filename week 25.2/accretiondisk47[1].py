...
number_of_annuli = 500  # Number of Annuli , N > 1 at all times 
...
sample_ratio = 10   # this parameter sets the time resolution of the time-series for each annulus
                    # for each annulus, our time step is the local viscous_timescale / sample_ratio
cycles       = 5    # number of viscous_timescales of the outermost (slowest) annulus to span
...
annuli_ratio = (R_out / R_in) ** (1.0 / (number_of_annuli - 1)) # determine the geometric ratio
...
def time_division(local_annuli):
    ...
    local_N_dt_annulus = np.empty(number_of_annuli,dtype=int) 
    ...
    for local_annulus in range(number_of_annuli):
        ...
        local_dt_annulus[local_annulus] = viscous_timescale(r) / sample_ratio 

        local_N_dt_annulus[local_annulus] = int(local_simulation_time /
...
def create_variable_array(local_N_dt_annulus):
    ...
    local_variable_array = np.empty(np.sum(local_N_dt_annulus))
...
def read_variable_array(array,local_annuli,local_N_dt_annulus,local_dt_annulus,
    ...
    #check to determine is an exact position has been specified, if not interpolate value between value1 and value2        
    if t != -1: 
        
        #interpolated result:
        value1 = array[local_annulus_start + t_pos]               # value before
        
        # check time position still in current annulus
        if t_pos + 1 < local_N_dt_annulus[local_annulus]:                     
            value2 = array[local_annulus_start + t_pos + 1]       # value after
        else:
            value2 = array[local_annulus_start]                    # value after if looping around 
        ...
        value = array[local_annulus_start + t_pos]
...
def M_dot(local_m_dot,local_annuli,local_N_dt_annulus,local_dt_annulus,local_annulus_t_pos_start):
    ...
    r = local_annuli[number_of_annuli-1]
    
    # For each time division in the array calculate M_dot and update the variable array
    for t_pos in range(local_N_dt_annulus[number_of_annuli-1]):
        
        value = M0_dot * (1.0 + read_variable_array(local_m_dot,local_annuli,local_N_dt_annulus,
    ...
    for local_annulus in range(number_of_annuli-2,-1,-1): # work backwards through array
        ...
        for t_pos in range(local_N_dt_annulus[local_annulus]):
            ...
            if t_offset < 0:    # If subtracting the offset makes the time negative loop round the timescale
                t_offset = t_offset + ((local_N_dt_annulus[local_annulus]-1)
                * local_dt_annulus[local_annulus])
                                          
            value = read_variable_array(local_M_dot,local_annuli,local_N_dt_annulus,
                                        local_dt_annulus,local_annulus_t_pos_start,t_offset,-1,r1
                                        ) * (1.0 + read_variable_array(local_m_dot,local_annuli,
...
def annulus_effective_temperature(local_M_dot,local_annuli,local_N_dt_annulus,
    ...
    for local_annulus in range(number_of_annuli):
        ...
        local_average_effective_temp = local_total_effective_temp / local_N_dt_annulus[local_annulus]
...
def plot_mass_flow(local_annuli,local_N_dt_annulus,local_dt_annulus,
    ...
    for local_annulus in [1,int(number_of_annuli/2),number_of_annuli-2]:
