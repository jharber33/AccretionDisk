...
N            = 200   # Number of Annuli , N > 1 at all times 
...
sample_ratio = 1000 # this parameter sets the time resolution of the time-series for each annulus
                    # for each annulus, our time step is the local viscous_timescale / sample_ratio
cycles       = 100   # number of viscous_timescales of the outermost (slowest) annulus to span
...
print (now.strftime("%Y-%m-%d %H:%M"), "Annuli Number=",N,"cycles=",cycles,"samples=",sample_ratio)
...
print("Status: disk (radius,intervals, divisions and start locations) set up")
...
record_time     = 100
inner_divisions = int(disk_time_divisions[0]-1)
time_series     = np.empty(int(inner_divisions / record_time))
counts          = np.empty(int(inner_divisions / record_time))
frequency       = 0.6e20

for i in range(int(inner_divisions/record_time)):
    observation_count = 0
    for j in range(record_time):
        t = ((i * record_time) + j) * disk_time_interval[0]
