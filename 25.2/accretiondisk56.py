...
R_in         = 10.0     # Radius of Central Body, eg. Black Hole, also the start of the disk  (Rg)
M_body       = 20.0 * SM  # Mass of Central Body in solar masses (kg)
R_out        = 10000.0  # Radial distance to edge of disk (Rg)
number_of_annuli = 1000   # Number of Annuli , N > 1 at all times 
print_plot_interval = int(number_of_annuli / 10)
...
def time_division(local_annuli):
    ...
    return local_N_dt_annulus, local_dt_annulus, local_annulus_tpos_start
...
def m_dot(local_annuli, local_N_dt_annulus, local_dt_annulus, local_annulus_tpos_start):
    ...
    if sinusidal:
        for local_annulus in range(number_of_annuli):
            ...
            for tpos in range(local_N_dt_annulus[local_annulus]):
...
def lightcurve(local_annuli,local_N_dt_annulus,local_dt_annulus,
                   local_M_dot,local_annulus_tpos_start,local_observation_time, local_frequency):
    ...
    for i in range(int(local_inner_time/local_observation_time)):  # loop through number of measurements
        ...
        if prints and ( (i / (5 * print_plot_interval)) == int(i / (5 * print_plot_interval)) ):
            print (i , " of ", int(local_inner_time/local_observation_time))
...
N_dt_annulus, dt_annulus, annulus_tpos_start = time_division(disk_annuli)
...
counts, time_axis = lightcurve(disk_annuli,N_dt_annulus,dt_annulus,
                   M_dot,annulus_tpos_start, observation_time, 0.125e19)
